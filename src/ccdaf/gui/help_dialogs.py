...
<p><b>The one shortcut to remember:</b> over the 3D view, <b>X</b> commits the
active tool's batch and drops points for the geodesic <i>snake</i> tools.</p>
<ol>
  ...
  <li><b>Correct by hand</b> — in <i>Manual correction</i>, pick a label, then
      select triangles (press <b>X</b> to commit) or use the snake.</li>
...
<table cellpadding="4">
  ...
  <tr><td><b>X</b></td><td>Commit the active tool's batch, or drop a snake
      point</td></tr>
</table>
<p><b>Who owns <code>X</code>:</b> it is shared by manual-correction selection,
the manual snake, and the PV-contour snake. The <i>Clipping active</i> checkbox
decides whether the key belongs to clipping or to manual correction, so only one
tool responds at a time.</p>
"""
